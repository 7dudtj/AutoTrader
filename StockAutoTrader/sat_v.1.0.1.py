...
def get_stock_balance(code):
    ...
    if code == 'ALL':
        post_message(token, '#coin', 'Account Name: ' + str(cpBalance.GetHeaderValue(0))+'\n'
                     'Payment balance : ' + str(cpBalance.GetHeaderValue(1))+'\n'
                     'Evaluation amount: ' + str(cpBalance.GetHeaderValue(3))+'\n'
                     'Evaluation profit and loss: ' + str(cpBalance.GetHeaderValue(4))+'\n'
...
def get_target_price(code):
    try:
        ...
        post_message(token, '#coin', "`get_target_price() -> exception! " + str(ex) + "`")
        return None
...
def get_movingaverage(code, window):
    try:
        ...
        post_message(token, '#coin', 'get_movingavrg(' + str(window) + ') -> exception! ' + str(ex))
        return None
    ...
    try:
        ...
        if current_price > target_price and current_price > ma5_price \
            ...
            if bought_qty > 0:
                ...
                post_message(token, '#coin', "`buy_etf(" + str(stock_name) + ' : ' + str(code) +
                       ") -> " + str(bought_qty) + "EA bought!" + "`")
    except Exception as ex:
        post_message(token, '#coin', "`buy_etf(" + str(code) + ") -> exception! " + str(ex) + "`")
...
def sell_all():
    try:
        ...
        post_message(token, '#coin', "sell_all() -> exception! " + str(ex))
# End of functions ---------------------------------------------------------------
...
if __name__ == '__main__':
    try:
        ...
        while True:
            ...
            if today == 5 or today == 6:  # Sat or Sun >> End program
            ...
            if t_start < t_now < t_sell:  # AM 09:05 ~ PM 03:15 : buy
                ...
                if t_now.minute == 30 and 0 <= t_now.second <= 5:
                    ...
                    time.sleep(5)
            if t_sell < t_now < t_exit:  # PM 03:15 ~ PM 03:20 : all sell
                if sell_all() == True:
                    post_message(token, '#coin', '`sell_all() returned True -> self-destructed!`')
                    sys.exit(0)
            if t_exit < t_now:  # PM 03:20 ~ :End program
                post_message(token, '#coin', '`self-destructed!`')
                sys.exit(0)
            time.sleep(3)
    except Exception as ex:
        post_message(token, '#coin', '`main -> exception! ' + str(ex) + '`')